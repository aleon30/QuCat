...
sonido_compuerta = pygame.mixer.Sound("sfx/get_gate.ogg")
sonido_salto = pygame.mixer.Sound("sfx/pixel_jump_sound.ogg")
sonido_medicion = pygame.mixer.Sound("sfx/medicion.ogg")
sonido_victoria = pygame.mixer.Sound("sfx/victoria.ogg")
sonido_game_over = pygame.mixer.Sound("sfx/game_over.ogg")
...
qubit = superposicion_liviana.qubit_inicial('0')
medido = False
puntos = 0
puntuacion = 0
...
def reiniciar_juego():
    ...
    qubit = superposicion_liviana.qubit_inicial('1' if objetivo_colapso == 0 else '0')


# Función principal asíncrona compatible con Pygbag
async def main():
    global estado, puntos, puntuacion, medido, salto_activo, velocidad_vertical
    global direccion_jugador, compuerta_actual, qubit, texto_colapso, objetivo_colapso
    global tiempo_inicio_victoria, tiempo_inicio_derrota
    ...
    while run:
        ...
        if estado == ESTADO_JUGANDO:
            ...
            if bloque.colliderect(jugador):
                ...
                qubit = superposicion_liviana.qubit_actual(qubit, compuerta_actual)
                compuerta_actual = random.choice(compuertas)

            if medidor.colliderect(jugador) and not medido:
                sonido_medicion.play()
                resultado = superposicion_liviana.colapsar_qubit(qubit)
                qubit = superposicion_liviana.qubit_inicial('0')
        ...
        if estado == ESTADO_MENU:
            ...
            texto_qubit = fuente.render(
                "Estado: " + formatear_estado(qubit[0], qubit[1]),
                True,
                BLANCO,
            )
